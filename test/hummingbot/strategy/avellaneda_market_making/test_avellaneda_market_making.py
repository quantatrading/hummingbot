...
class AvellanedaMarketMakingUnitTests(unittest.TestCase):
    ...
    def test_reservation_price_includes_positive_drift_bias_on_uptrend(self):
        self.config_map.drift_enabled = True
        self.config_map.drift_confirm_secs = 0
        self.config_map.drift_hysteresis_secs = 0
        self.config_map.drift_window_short_secs = 5
        self.config_map.drift_window_long_secs = 10
        self.config_map.drift_window_vol_secs = 10
        self.config_map.drift_z_threshold = Decimal("0.2")
        self.config_map.drift_kappa = Decimal("0.35")
        self.config_map.drift_spread_adjust_enabled = False
        self.strategy.update_from_config_map()

        # Keep model terms available
        self.simulate_low_volatility(self.strategy)
        self.simulate_high_liquidity(self.strategy)
        self.strategy.measure_order_book_liquidity()
        ...
        for i in range(1, 80):
            ...
            self.strategy.collect_market_variables(ts + i)
            self.strategy.calculate_reservation_price_and_optimal_spread()

        self.config_map.drift_enabled = False
        self.strategy.update_from_config_map()
        self.strategy.calculate_reservation_price_and_optimal_spread()
        reservation_without_drift = self.strategy.reservation_price

        self.config_map.drift_enabled = True
        self.strategy.update_from_config_map()
        self.strategy.calculate_reservation_price_and_optimal_spread()
        reservation_with_drift = self.strategy.reservation_price

        self.assertGreater(reservation_with_drift, reservation_without_drift)
