...
class ToxicityGateTests(unittest.TestCase):
    def test_ewma_updates_with_known_dt(self):
        ...
        gate.update(now=5.0, mid_price=99.0)  # -100 bps
        self.assertAlmostEqual(-100.0, gate.ewma_adv_bps_by_horizon()[5], places=6)

        gate.on_fill("BUY", 100.0, 10.0)
        gate.update(now=15.0, mid_price=98.0)  # -200 bps, dt = 10s => alpha=0.5
        self.assertAlmostEqual(-150.0, gate.ewma_adv_bps_by_horizon()[5], places=5)
